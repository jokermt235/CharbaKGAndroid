...
import org.exesoft.charbakg.View.MrsOffspringFormActivity;

import java.util.ArrayList;
import java.util.Map;

public class MrsOffspringController {
    private static String TAG = "MrsOffspringController";
    private static  int sum = 0;

    public static void save(final MrsOffspringFormActivity activity, Map<String, Object> mrs){
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        FirebaseUser user  = FirebaseAuth.getInstance().getCurrentUser();
        if(user != null) {
            mrs.put("phone",user.getPhoneNumber());
            db.collection("mrs_offspring")
                    .add(mrs)
    ...
    public static void offspring(final MRSActivity activity, long dateFrom, long dateTo) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        ...
        sum = 0;
        if(user != null) {
            db.collection("mrs_offspring").whereGreaterThan("added",dateFrom).
